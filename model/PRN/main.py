...
# To test the model on images with reflection
    ...
    test_path = ["../dataset/I/"] #["./test_images/real/"]
    val_names=prepare_data_test(test_path)
    os.system("mkdir ../output/PRN")
    path = "../dataset/B/"
    for filename in os.listdir(path):
        print(os.path.join(path, filename))
        print("../output/PRN/B_"+filename)
        copyfile(os.path.join(path, filename), "../output/PRN/B_"+filename)
    ...
    for val_path in val_names:
        ...
        cv2.imwrite("../output/PRN/T_" + testind + ".jpg", np.uint8(output_image_t[0,:,:,0:3])) # T
        cv2.imwrite("../output/PRN/R_" + testind + ".jpg", np.uint8(output_image_r[0,:,:,0:3])) # R

psnr_ssim.quality_assess_SM3("../output/PRN", "../output/PRN", 'B_', 'T_')
